...
const MinimumDosageBody = Class.create({
  name: "minimumDosageBody",
  fields: {
    nA: {
      type: Boolean,
      default: function () {
    ...
    beforeSave(e) {
      ...
      if (!unit || !possibleUnits.includes(unit)) {
        throw new Meteor.Error(
          "validation-error",
          "Minimum Dosage unit must be one of " + possibleUnits.join(", ")
        );
      }
      ...
      if (!frequencyPeriod || !possiblePeriods.includes(frequencyPeriod)) {
        throw new Meteor.Error(
          "validation-error",
          "Minimum Dosage frequency period must be one of " +
      ...
      if (
        ...
        throw new Meteor.Error(
          "validation-error",
          "Minimum Dosage frequency duration period must be one of " +
            possiblePeriods.join(", ")
        );
      }
...
export const MinimumDosage = FieldObject.inherit({
  name: "minimumDosage",
  collection: FieldObjects,
  fields: {
    body: MinimumDosageBody,
  },
});
